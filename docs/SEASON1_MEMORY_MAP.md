...
1. `../원고/수정본_txt/Chapter 01 - Ash.txt`부터 `Chapter 46 - Residue.txt`까지의 영문 원고
2. 현재 게임의 `data/`, `scenes/`, dialogue JSON, VN JSON, progression
3. GDD, 세션 로그, 이전 분석

영문 원고는 1~46장이 번호 누락 없이 모두 존재한다. 39장은 파일명과 본문 제목 모두 `The Registrar's Hand`이고 파일명에 `DRAFT`가 남아 있으므로, 사건은 현재 Season 1 정사로 읽되 문구 안정성은 낮다고 표시한다.

한국어판 폴더는 계속 갱신 중이다. 2026-08-23 재확인 시 1~32장 파일이 존재하지만 32장은 영문 269줄 대비 139줄로 진행 중이며 33~46장은 아직 없다. 따라서 시스템 ID와 사건 판정은 안정적으로 완성된 영문 46장을 기준으로 한다.
