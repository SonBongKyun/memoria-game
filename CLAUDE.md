...
- `EliaDiary` — 엘리아 일지 + 전투 기술 (일지 8항목, 비연소 기술 4종, 쿨다운 시스템)
- `TutorialHints` — 튜토리얼 힌트 (5종 상황별 첫 발생 가이드 팝업)
- `StoryLog` — 회상 기록 (지나간 대사 백로그, L키/일시정지 메뉴, 읽은 대사 등록부 → 빨리감기 범위 판단)

> 이 목록은 초기 세션 기준이다. 이후 추가된 오토로드/유틸의 전체 목록은 `project.godot`의 `[autoload]`를 확인할 것.
...
- **S49 완료 (2026-04-10):** 10챕터 확장 Phase 1 — 6→10챕터 구조 재편, Ch3 Belt Waystation(토비아스/백서), Ch4 Drift Shelter(읽기 저하/앵커링), 전체 플래그·대화·전환 재번호, 대화 ~1050줄
- **S50 완료 (2026-04-12):** 10챕터 확장 Phase 2 — Ch7 Seam Outskirts(세이블 진실/에코 셸/시련), Ch8 Forgotten Forest(기억 기생 숲/유령/링 이론), Ch9 Colorless Waste(무색 황무지/메모리 나침반/카이로스 대면), 전투 능력 3종(stun/reflect/charge), 전환 체인 완성, 대화 ~1325줄
- **S51 완료 (2026-04-12):** 게임성 대폭 업그레이드 — Memory Decay/Erosion(기억 침식), Memory Echo(연소 잔류 효과 7종), Battle Stance(전투 자세 3종), Void Corruption Modifiers(인카운터 수정자 12종), Elia Diary(일지+전투 기술 4종), Memory Resonance(탐색 공명 18지점)
...
- **S55~S209:** 위 요약은 S54 시점에서 멈춰 있다. 이후 세션(파트 II·III 확장, VN 전환, 삽화 450장, 한국어화, 하이브리드 2D/3D 전투, 전투 커맨드 덱, 인벤토리 QoL, 전투 무대/템포/회상 기록 등)의 기록은 **`SESSION_LOG.md`가 정본**이다. 세션 시작 시 그쪽을 먼저 읽을 것.
- **상태(LEGACY S209 기준):** 당시 구현 규모와 검증 기록이다. 현재 Season 1 canon과 gameplay checkpoint는 문서 최상단 계약을 따른다.
