...
func _build_ui() -> void:
	...
	if _resolved_battle_bg_image != "":
		...
		# S249: 이 값을 지역별로 푸는 것을 시도했다가 되돌렸다. 같은 표본 띠에서
		# 필드 맵은 밝기 0.108~0.489(4.4배 폭)인데 전투는 0.073~0.092(1.3배)라,
		# 전투가 시작되면 장소가 지워진다는 계측이 있었기 때문이다.
		# 그러나 후퇴량을 0.50 → 0.55 → 0.62로 올려도 띠 밝기는 0.0833 → 0.0848 →
		# 0.0872로 5% 남짓만 움직였다. 배경 CG는 이 띠 밝기의 12%만 기여한다.
		# 지배 항은 아직 못 찾았고, 못 찾은 채로 값을 바꾸지 않는다.
